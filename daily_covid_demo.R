...
csvlist = list.files('temp/PredictorVW/')
...
ffd = read.csv('F-F_Research_Data_Factors_daily.CSV', skip=4) %>%
  rename(date = X) %>%
  as_tibble() %>%
...
small = all %>% filter(date >= '2020-01-01') %>%   
...
ggplot(
  small %>% filter(port %in% c('OPLeverage', 'EarningsSurprise','BMdec')                   )
         , aes(x=date,y=cret)) +
...
temp = 1
ggsave(filename = paste0("covid_anom_select.png")
...
temp = 1
ggsave(filename = paste0("covid_anom_all.png")
       , width = 10*temp, height = 6*temp)
